...
class GitAutoDeploy(BaseHTTPRequestHandler):
    ...
    CONFIG_FILEPATH = './gitdeploy.conf.json'
    config = None
    quiet = False
    daemon = False
    ...
    def do_POST(self):
        ...
        if event != 'pullrequest:fulfilled' and event != 'repo:push':
            if not self.quiet:
                print event
                print 'We only handle pullrequest:fulfilled and repo:push events'
            self.respond(304)
            return

        self.respond(204)

        urls = self.parseRequest(event)
        for url in urls:
    ...
    def parseRequest(self,event):
        length = int(self.headers.getheader('content-length'))
        body = self.rfile.read(length)
        payload = json.loads(body)
        if event ==	'pullrequest:fulfilled':
                self.branch = payload['pullrequest']['destination']['branch']['name']
        else:
